/**
 * useGraph.js — Polls GET /graph every 15s and computes BFS layout.
 *
 * Moved from: frontend/src/hooks/useGraph.js
 * New location: apps/dashboard/src/hooks/useGraph.js
 *
 * No logic changes. The BFS rank-based layout algorithm is intentionally
 * kept here (not in a separate lib) because it's tightly coupled to the
 * node shape expected by DependencyGraph. If it ever needs to be reused,
 * extract to utils/layout.js at that point.
 */
  ...
  useEffect(() => {
    ...
    async function fetchGraph() {
      try {
        ...
        const campusEdges = validEdges.filter(e => campusIds.has(e.source) && campusIds.has(e.target));

        const layouted  = computeLayout(campus, campusEdges).map(n => ({ ...n, data: { ...n.data, isSys: false } }));
        const sysNodes  = sys.map(n => ({ ...n, position: { x: 0, y: 0 }, data: { ...n.data, isSys: true } }));
