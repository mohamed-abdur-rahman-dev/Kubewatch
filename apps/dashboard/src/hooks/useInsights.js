/**
 * useInsights.js — Polls GET /insights every 60s for Ollama, no auto-poll for OpenAI.
 *
 * Moved from: frontend/src/hooks/useInsights.js
 * New location: apps/dashboard/src/hooks/useInsights.js
 *
 * No logic changes. Returns structured fields (insight, blastRadius, action)
 * that the backend now returns directly — no frontend parsing needed.
 */
...
export default function useInsights(provider = 'ollama', refreshTrigger = 0) {
  ...
  // Tick the elapsed-seconds counter while loading (drives the LoadingState message)
  useEffect(() => {
    if (loading) {
  ...
  useEffect(() => {
    ...
    // OpenAI costs money — only fetch when user explicitly refreshes.
    // Ollama is free/local — auto-refresh every minute.
    let id = null;
    if (provider !== 'openai') {
