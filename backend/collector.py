...
from models import PodMetric

logger = logging.getLogger(__name__)

CPU_LIMIT_DEFAULT_M = 1000
MEM_LIMIT_DEFAULT_MB = 512
...
def collect_pods() -> List[PodMetric]:
    ...
    # --- fetch metrics (optional, retry up to 3x on transient 503) ---
    metrics_map: dict = {}
    for attempt in range(3):
        try:
            metrics_raw = custom.list_cluster_custom_object(
    ...
    for pod in pod_items:
        ...
        # resource limits from first container
        cpu_limit_m = CPU_LIMIT_DEFAULT_M
        mem_limit_mb = MEM_LIMIT_DEFAULT_MB
        try:
            limits = pod.spec.containers[0].resources.limits or {}
            if "cpu" in limits:
                cpu_limit_m = _parse_cpu(limits["cpu"]) or CPU_LIMIT_DEFAULT_M
            if "memory" in limits:
                mem_limit_mb = _parse_mem(limits["memory"]) or MEM_LIMIT_DEFAULT_MB
        except Exception:
            pass
        ...
        pods.append(PodMetric(
            ...
            labels=labels,
            status=_compute_status(cpu_pct, mem_pct, restarts),
        ))
...
def collect_services() -> list:
    """Return raw service list from all namespaces."""
    try:
        _load_k8s_config()
...
def collect_events() -> list:
    """Return Warning events from the last 10 minutes."""
    try:
        _load_k8s_config()
